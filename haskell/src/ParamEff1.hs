...
-- http://okmij.org/ftp/Haskell/extensible/index.html#extext
...
instance Functor (Eff f) where
  ...
  fmap f (Impure m k) = Impure m (fmap f . k)
...
instance Functor (EffP f s1 s2) where
  fmap f (PureP x)     = PureP (f x)
  fmap f (ImpureP m k) = ImpureP m (fmap f . k)
